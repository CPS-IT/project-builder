...
> :arrow_right: Read more at [`Processing build steps`](build-steps.md).
...
The complete JSON schema can be found at [`resources/build-artifact.schema.json`](../resources/build-artifact.schema.json).

> :bulb: Build artifacts are not generated by default. Instead, template packages
> must be explicitly configured to create such files. This can be done by including
> the [`GenerateBuildArtifactStep`](build-steps.md#generate-build-artifact)
