...
final class CollectBuildInstructionsStep extends AbstractStep
{
    ...
    private Interaction\QuestionInteraction $questionInteraction;
    private Interaction\SelectInteraction $selectInteraction;
    private Interaction\StaticValueInteraction $staticValueInteraction;

    public function __construct(
        ExpressionLanguage\ExpressionLanguage $expressionLanguage,
        IO\Messenger $messenger,
        Builder\Generator\Step\Interaction\QuestionInteraction $questionInteraction,
        Builder\Generator\Step\Interaction\SelectInteraction $selectInteraction,
        Builder\Generator\Step\Interaction\StaticValueInteraction $staticValueInteraction
    ) {
        parent::__construct();
        ...
        $this->staticValueInteraction = $staticValueInteraction;
    }
    ...
    private function applySubProperty(Builder\Config\ValueObject\SubProperty $subProperty, Builder\BuildResult $buildResult): void
    {
        ...
        switch ($subProperty->getType()) {
            case 'staticValue':
                $value = $this->staticValueInteraction->interact($subProperty, $instructions);

                break;

            case 'select':
                $value = $this->selectInteraction->interact($subProperty, $instructions);
            ...
            case 'question':
                $value = $this->questionInteraction->interact($subProperty, $instructions);

                break;

            default:
                throw Exception\InvalidConfigurationException::create($subProperty->getType());
        }
