...
final class Generator
{
    ...
    {
        $step = new Builder\Generator\Step\CleanUpStep(new Filesystem\Filesystem());
        $step->run($result);
    }
    ...
    private function moveTemplateDirectory(): void
    {
        $configFile = $this->config->getDeclaringFile();
        $sourceDirectory = dirname($configFile);
        $targetDirectory = Helper\FilesystemHelper::getNewTemporaryDirectory();

        $this->filesystem->mirror($sourceDirectory, $targetDirectory);
        $this->filesystem->remove($sourceDirectory);

        $this->config->setDeclaringFile(Filesystem\Path::join($targetDirectory, basename($configFile)));
    }
}
